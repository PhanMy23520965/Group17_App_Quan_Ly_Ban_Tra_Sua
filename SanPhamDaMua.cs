...
namespace TraSuaApp.View
{
    public partial class SanPhamDaMua : Form
    {
        ...
        {
            // Khai báo MaKH
            string maKH = Program.MaKH;  // Mã khách hàng cần lấy

            // Lấy tất cả đơn hàng của khách hàng từ Firestore
            var orders = await db.Collection("DonHang")
                                 .WhereEqualTo("MaKH", maKH)
                                 .GetSnapshotAsync();

            // Mảng lưu trữ thông tin sản phẩm đã mua
            var purchasedProducts = new List<(string TenSP, double Gia, string HinhAnh, string TrangThai)>();

            foreach (var order in orders)
            {
                // Lấy subCollection ChiTietDonHang từ mỗi đơn hàng
                ...
                foreach (var item in chiTietDonHang)
                {
                    ...
                    if (productDoc.Exists)
                    {
                        ...
                        // Thêm vào danh sách sản phẩm đã mua
                        purchasedProducts.Add((name, price, imageUrl, trangthai));
                    }
                }
            }

            // Hiển thị danh sách sản phẩm đã mua
            foreach (var (name, price, imageUrl, trangthai) in purchasedProducts)
            {
