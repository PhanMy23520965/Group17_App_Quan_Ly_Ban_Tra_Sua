    ...
    public partial class Dangky : Form
    {
        ...
        public async Task DangKyTaiKhoanVaKhachHang(string email, string matKhau, string hoTen, string soDienThoai)
        {
            try
            {
                ...
                // 2.Tạo khách hàng
                // Truy vấn tất cả mã khách hàng hiện có
                var khachHangSnapshot = await db.Collection("KhachHang").GetSnapshotAsync();

                // Tìm mã khách hàng lớn nhất
                maxSo = 0;
                foreach (var doc in khachHangSnapshot.Documents)
                {
                    string maKH = doc.Id; // ví dụ: KH001
                    if (maKH.StartsWith("KH") && int.TryParse(maKH.Substring(2), out int so))
                    {
                        if (so > maxSo) maxSo = so;
                    }
                }
                ...
                // Tạo mã đơn hàng mới
                soMoi = maxSo + 1;
                string maKHMoi = "KH" + soMoi.ToString().PadLeft(3, '0');
                ...
                await DBServices.POST(maKHMoi, "KhachHang", khachHang);

                // 3. Thêm mã khuyến mãi cho từng khách hàng

                // Trỏ đến document khách hàng
                DocumentReference khRef = db.Collection("KhachHang").Document(maKHMoi);

                // Trỏ đến collection con: KhachHang/{MaKH}/KhuyenMai
                CollectionReference kmUserCol = khRef.Collection("KhuyenMai");

                // Lấy tất cả document trong collection KhuyenMai gốc
                var khuyenmaiSnapshot = await db.Collection("KhuyenMai").GetSnapshotAsync();

                foreach (var doc in khuyenmaiSnapshot.Documents)
                {
                    // Lấy DocumentReference của từng khuyến mãi
                    DocumentReference kmRef = doc.Reference;

                    // Lưu document con trong KhachHang/{MaKH}/KhuyenMai với đúng ID như khuyến mãi gốc
                    var khuyenMaiUser = new Dictionary<string, object>
                    {
                        { "MaKM", kmRef },        // Reference đến KhuyenMai gốc
                        { "DaSuDung", false }     // Ban đầu là chưa sử dụng
                    };

                    // Dùng chính ID của KhuyenMai gốc để đặt document con (ví dụ: KM001)
                    await kmUserCol.Document(doc.Id).SetAsync(khuyenMaiUser);
                }
